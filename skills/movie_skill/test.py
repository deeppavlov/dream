...
def test_one_step_responses():
    ...
    assert response[1:] == [0.98, {}, {}, {'actor': [['Chris Evans', 'very_positive']]}], print(response)

    input_data = get_input_json("test_configs/check_director_more_important_than_actor.json")
    response = requests.post(url, json=input_data).json()[0]
    assert response[1:] == [0.98, {}, {}, {'director': [['Quentin Tarantino', 'very_positive']]}], print(response)
    ...
    response = requests.post(url, json=input_data).json()[0]
    assert response[1:] == [0.98, {}, {}, {'actor': [['Quentin Tarantino', 'positive']]}], print(response)
    ...
    response = requests.post(url, json=input_data).json()[0]
    assert response[1:] == [0.98, {}, {}, {'actor': [['Brad Pitt', 'very_positive']]}], print(response)
    ...
    response = requests.post(url, json=input_data).json()[0]
    assert response[1:] == [0.98, {}, {}, {'actor': [['Brad Pitt', 'very_positive'],
                                                     ['Chris Evans', 'very_positive']]}], print(response)
