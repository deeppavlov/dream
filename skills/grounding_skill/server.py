...
from scenario import GroundingSkillScenario
...
scenario = GroundingSkillScenario()
logger.info("Scenario done")


@app.route("/grounding_skill", methods=["POST"])
def respond():
    st_time = time.time()
    ...
    logger.info(f"grounding_skill exec time: {total_time:.3f}s")
