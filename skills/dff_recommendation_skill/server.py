...
INFILLING = os.environ["INFILLING"]
...
def is_container_running():
    try:
        requested_data = {"texts": ' Boo _'} #возможно здесь изменятся входные данные
        response = requests.post(INFILLING, json={"dialog_contexts": [requested_data]}, timeout=5) #посмотреть как это же делается в инфиллинге, поменяется урл и реквест дата
