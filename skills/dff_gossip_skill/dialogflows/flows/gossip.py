...
def get_people_for_topic(cobot_topic):
    ...
    peoples = [
        list(x.get("People", []))
        for x in common_gossip.TOPICS_TO_PEOPLE_MAPPINGS
        if x.get("Topic", "") == cobot_topic
    ]
    peoples = list(set(sum(peoples, [])))
    # wikidata-based list
    top_people_from_wiki = utils.get_top_people_from_wiki_for_cobot_topic(
        cobot_topic, peoples
    )
...
def save_mentioned_person(vars, person, judgement, share_memory_key):
    if person:
        ...
        are_judgements = [
            x.get("Judgement", "") == judgement for x in all_mentioned_people
        ]
        ...
        state_utils.save_to_shared_memory(
            vars, **{share_memory_key: all_mentioned_people}
        )
...
def get_mentioned_people(
    vars,
    share_memory_key="",
    judgements=["Liked", "Disliked", "Not Interested", "Other"],
):
    shared_memory = state_utils.get_shared_memory(vars)
    # obtaining a list of previously mentioned people
    all_mentioned_people = shared_memory.get(share_memory_key, [])
    if all_mentioned_people:
        peoples = [
            list(x.get("People", []))
            for x in all_mentioned_people
            if x.get("Judgement", "") in judgements
        ]
        peoples = list(set(sum(peoples, [])))
        return peoples
    else:
...
def get_fresh_person_for_topic(vars, cobot_topic):
    all_mentioned_people = set(
        get_mentioned_people(vars, share_memory_key="people_mentioned_by_bot")
    )
    topic_people = [
        list(i.get("People", []))
        for i in common_gossip.TOPICS_TO_PEOPLE_MAPPINGS
        if i.get("Topic", "") == cobot_topic
    ]
...
def get_people_related_to_bot_mentioned_ones(vars, user_mentioned_person):
    ...
    people_mentioned_and_liked_by_bot = get_mentioned_people(
        vars, "people_mentioned_by_bot", ["Liked", "Disliked"]
    )

    for person in people_mentioned_and_liked_by_bot:
        relationship = utils.get_relationship_between_two_people(
            user_mentioned_person, person
        )
        if relationship:
...
def get_people_related_to_user_mentioned_ones(vars, user_mentioned_person):
    ...
    people_mentioned_and_liked_by_user = get_mentioned_people(
        vars, "people_mentioned_by_user", ["Liked", "Disliked"]
    )
...
def get_news_for_topic(vars, cobot_topic):
    ...
    mentioned_people = get_mentioned_people(
        vars, share_memory_key="people_mentioned_by_bot"
    )
    people = [person for person in people if person not in mentioned_people]

    if people:
        person = random.choice(people)
        curr_news = general_this_news.get_news_about_topic(
            person, NEWS_API_ANNOTATOR_URL
        )
        ...
        if curr_news and "content" in curr_news and "title" in curr_news:
            ...
            if person.lower() in content.lower():
                ...
                filtered_content = get_phrase_about_person_in_content(
                    person.lower(), content.lower()
                )
    ...
    topic_news = [
        list(i["News"])
        for i in this_news.TEMPORARY_NEWS_FOR_COBOT_TOPICS
        if i.get("Topic", "") == cobot_topic
    ]
...
def get_random_judgement_for_emotion(emotion):
    judgements = [
        list(x.get("People", []))
        for x in this_gossip.TARGET_JUDGEMENTS_FOR_EMOTION
        if x["Emotion"] in emotion
    ]
...
def get_supported_topics(vars):
    topics = common_utils.get_topics(
...
def set_people_jobs(vars, celebrity_name, core_jobs, other_jobs, mentioned_jobs):
    ...
    mentioned_jobs = (
        people_jobs.get(celebrity_name, {}).get("Mentioned_Jobs", []) + mentioned_jobs
    )
    mentioned_jobs = list(set(mentioned_jobs))
    people_jobs[celebrity_name] = {
        "Jobs": core_jobs,
        "Other_Jobs": other_jobs,
        "Mentioned_Jobs": mentioned_jobs,
    }
...
def get_celebrity_from_uttr(vars, exclude_types=False, use_only_last_utt=False):
    ...
    human_utterance = state_utils.get_last_human_utterance(vars)
    logger.debug(
        f'Calling get_celebrity_from_uttr on {human_utterance["text"]} {exclude_types} {use_only_last_utt}'
    )
    ...
    (
        ...
        matching_types,
        mismatching_types,
    ) = common_gossip.celebrity_from_uttr(human_utterance)
    ...
    mismatching_types = [
        type_ for type_ in mismatching_types if type_ not in mentioned_jobs
    ]
    ...
    set_people_jobs(
        ...
        matching_types,
        mismatching_types,
        mentioned_jobs + [mentioned_job],
    )
    logger.debug(
        f"Answer for get_celebrity exclude_types {exclude_types} : {celebrity_name} {mentioned_job}"
    )
...
def sys_celebrity_found_request(ngrams, vars, use_only_last_utt=True):
    ...
    person, occupation = get_celebrity_from_uttr(
        vars, use_only_last_utt=use_only_last_utt
    )
...
def get_celebrity_prompt(vars, person):
    ...
    if person:
        ...
        if matching_jobs:
            logger.info(
                f"get_celebrity_prompt: matching jobs! just was celebrity prompt? {just_was_celebrity_prompt}"
            )
            is_actor = "actor" in " ".join(matching_jobs)
            prompt_candidate = (
                f"{person} is an amazing {matching_jobs[0]}! "
            ...
            if (
                not just_was_celebrity_prompt
                and matching_jobs
                and we_not_repeat_start_prompt
            ):
                logger.debug("start prompt")
                prompt = prompt_candidate
            elif (
                just_was_celebrity_prompt
                and matching_jobs
                and mismatching_jobs
                and actor_asking not in last_bot_uttr
            ):
                logger.info(
                    "get_celebrity_prompt: just was celebrity prompt and actor:"
                )
                ...
                set_people_jobs(
                    vars, person, matching_jobs, mismatching_jobs, mentioned_jobs
                )
            elif (
                just_was_celebrity_prompt
                and matching_jobs
                ...
                logger.info(
                    "get_celebrity_prompt: just was celebrity prompt and non-actor:"
                )
                prompt = get_cobot_fact(person, used_celeb_prompts)
                logger.debug(f"Cobot prompt {prompt}")
            if prompt:
...
def sys_topic_to_event_request(ngrams, vars):
    ...
    bot_utterance = state_utils.get_last_bot_utterance(vars)
    flag = (
        bool(get_supported_topics(vars)) and talk_about_gossip_request(ngrams, vars)
    ) or (
...
def usr_topic_to_event_response(vars):
    ...
    try:
        ...
        if selected_topics:
            cobot_topic = random.choice(
                list(selected_topics)
            )  # "Entertainment_Movies" # for the time being
        else:
        ...
        logger.debug(
            f"person = {person}, news_title = {news_title}, news_content = {news_content}, "
        )
        ...
        if person and person.lower() in news_title.lower():
            ...
            logger.debug(
                f"News about {person} : It's said that {news_content}: {news_title}"
            )
            event = news_content
        else:
        ...
        questions = (
            questions if topic else [i for i in questions if "target_topic" not in i]
        )
        questions = (
            questions if event else [i for i in questions if "target_event" not in i]
        )
...
def sys_no_or_yes_request(ngrams, vars):
    ...
    sf_type, sf_confidence = utils.get_speech_function_for_human_utterance(
        human_utterance
    )
...
def usr_event_to_person_response(vars):
    ...
    try:
        ...
        state_utils.save_to_shared_memory(
            vars, bot_emotion_towards_current_person=target_emotion_type
        )
        ...
        occupation = utils.get_occupation_for_person(
            current_person, current_cobot_topic, body
        )
...
def get_cobot_fact(celebrity_name, given_facts):
    ...
    if answer is None:
        error_message = (
            f"Answer from random fact or fact about {celebrity_name} not obtained"
        )
...
def usr_not_interested_in_person_response(vars):
    ...
    try:
        ...
        # occupation
        occupation = utils.get_occupation_for_person(
            person, current_cobot_topic, prompt
        )

        fake_utterance = (
            f"I like to learn more about {occupation} {person} {human_topic}"
        )
...
patterns_creative_topics = [
    "Entertainment_Movies",
    "Entertainment_Books",
    "Entertainment_Music",
]
creative_topics_patterns_re = re.compile(
    "(" + "|".join(patterns_creative_topics) + ")", re.IGNORECASE
)
...
def usr_agrees_abt_person_response(vars):
    ...
    try:
        ...
        if len(current_person_occupation) == 0:
            current_person_occupation = utils.get_basic_occupation_for_topic(
                current_cobot_topic
            )
        ...
        bot_emotion_towards_current_person = shared_memory.get(
            "bot_emotion_towards_current_person", "Liked"
        )
        # obtaining judgement
        bot_judgement = get_random_judgement_for_emotion(
            bot_emotion_towards_current_person
        )
        ...
        if age != 0:
            if age < 25:
                ...
                prompt = random.choice(
                    this_gossip.REACTION_TO_YOUNG_AGE[
                        bot_emotion_towards_current_person
                    ]
                )
                prompt = replace_occupation(
                    prompt, bot_judgement, current_person_occupation
                )
                prompt = replace_gender(prompt, gender)

        # ASPECT #2: PERSONAL RELATIONSHIPS (SPOUSE/PARTNER)
        if not prompt:
            spouse, partner = utils.get_spouse_or_partner_person(
        ...
        if not prompt:
            is_creative_person = bool(
                re.search(creative_topics_patterns_re, current_cobot_topic)
            )
            # obtaining
            if is_creative_person:
                ...
                (
                    ...
                    albums,
                    notable_works,
                ) = utils.get_notable_works_for_creative_person(
                    current_person, fake_utterance
                )
                ...
                if notable_work:
                    ...
                    body = ""
                    prompt = random.choice(
                        this_gossip.REACTION_TO_CREATIVE_WORK[
                            bot_emotion_towards_current_person
                        ]
                    )
        ...
        if not prompt:
            if "Sports" in current_cobot_topic:
                ...
                # TODO : oh my god
                team_name = "[[]]"
                sport, teams = utils.get_teams_for_sportsperson(
                    current_person, fake_utterance
                )
                ...
                if "[[]]" in str(team_name):
                    ...
                    body = ""
                    prompt = random.choice(
                        this_gossip.GENERIC_REACTION_TO_SPORT[
                            bot_emotion_towards_current_person
                        ]
                    )
                    prompt = prompt.replace("target_sport_name", sports_kind)
        ...
        if not prompt:
            ...
            logger.info(
                f"usr_agrees_abt_person_response: CELEBRITY, suggested prompt: {prompt}"
            )
            if not prompt:
                prompt = ""
            else:
...
def usr_says_opinion_abt_person_response(vars):
    ...
    try:
        ...
        save_mentioned_person(
            vars, current_person, judgement, "people_mentioned_by_user"
        )

        prompt = random.choice(
            this_gossip.REACTION_TO_USER_OPINION_ABOUT_PERSON[judgement]
        )
...
def sys_mentions_another_person_request(ngrams, vars):
    ...
    found_celebrity, _ = get_celebrity_from_uttr(
        vars, use_only_last_utt=use_only_last_utt
    )
    ...
    other_mentioned_people = [
        people
        for people in mentioned_by_user_people
        if str(people).lower() != current_person.lower()
    ]
...
def usr_mentions_another_person_response(vars):
    ...
    try:
        ...
        use_only_last_utt = True
        found_celebrity, occupation = get_celebrity_from_uttr(
            vars, use_only_last_utt=use_only_last_utt
        )
        ...
        if found_celebrity:
            ...
            if str(user_mentioned_person).lower() in str(current_person).lower():
                logger.info(
                    f"just mentioned person {user_mentioned_person} is the current_one: {current_person}"
                )
                if utils.is_speech_function_demand_opinion(vars):
                    if current_person in get_mentioned_people(
                        vars, "people_mentioned_by_bot", ["Liked"]
                    ):
                        body = random.choice(
                            this_gossip.SIMPLE_OPINION_ABOUT_LIKED_PERSON_PREVIOUSLY_MENTIONED_BY_BOT
                        )
                        fake_utterance = (
                            f"I like to learn more about {user_mentioned_person}"
                        )
                        gender, age = utils.get_gender_age_person(
                            user_mentioned_person, fake_utterance
                        )
                        gender_is = (
                            utils.get_human_readable_gender_statement_current_is(gender)
                        )
                        body = body.replace("target_gender_is", gender_is)
                    elif current_person in get_mentioned_people(
                        vars, "people_mentioned_by_bot", ["Disliked"]
                    ):
                        body = random.choice(
                ...
                if utils.is_speech_function_express_opinion(vars):
                    body = random.choice(
                        this_gossip.SIMPLE_REACTION_TO_PERSON_PREVIOUSLY_MENTIONED_BY_BOT
                    )
            # path #2: mentioned person is the one mentioned by bot before
            elif user_mentioned_person in get_mentioned_people(
                vars, share_memory_key="people_mentioned_by_bot"
            ):
                if utils.is_speech_function_demand_opinion(vars):
                    if user_mentioned_person in get_mentioned_people(
                        vars, "people_mentioned_by_bot", ["Liked"]
                    ):
                        body = random.choice(
                            this_gossip.SIMPLE_OPINION_ABOUT_LIKED_PERSON_PREVIOUSLY_MENTIONED_BY_BOT
            ...
            # path #3: mentioned person is the one mentioned by user before
                ...
                vars, "people_mentioned_by_user"
            ):
                if utils.is_speech_function_demand_opinion(vars):
                    body = random.choice(
                        this_gossip.SIMPLE_OPINION_ABOUT_PERSON_PREVIOUSLY_MENTIONED_BY_USER
                    )
                if utils.is_speech_function_express_opinion(vars):
                    body = random.choice(
                        this_gossip.SIMPLE_REACTION_TO_PERSON_PREVIOUSLY_MENTIONED_BY_USER
                    )
            # path #4: mentioned person is the new one
            else:
                bot_mentioned_people_related_to_new_ones = (
                    get_people_related_to_bot_mentioned_ones(
                        vars, user_mentioned_person
                    )
                )
                user_mentioned_people_related_to_new_ones = (
                    get_people_related_to_user_mentioned_ones(
                        vars, user_mentioned_person
                    )
                )
                ...
                # generic case, just a new person
                    ...
                    body = utils.get_not_used_and_save_reaction_to_new_mentioned_person(
                        vars
                    )
                    fake_utterance = (
                        f"I like to learn more about {user_mentioned_person}"
                    ...
                    # gender_is = utils.get_human_readable_gender_statement_current_is(gender)
                    body = body.replace(
                        "target_gender_is", user_mentioned_person + " is "
                    )

                    # saving person to the list of people mentioned by bot, for now with "Like" judgement
                    save_mentioned_person(
                        vars, current_person, "Liked", "people_mentioned_by_bot"
                    )
            ...
            if "negative" in sentiment:
                ...
                judgement = (
                    "Liked"  # temporary override until we'll get templates for neutral
                )

            save_mentioned_person(
                vars, user_mentioned_person, judgement, "people_mentioned_by_user"
            )

            # saving current user's emotion towards the currently discussed person
            state_utils.save_to_shared_memory(
                vars, bot_emotion_towards_current_person=judgement
            )
            ...
            logger.info(
                f"sys_mentioned_another_person: setting current_person to {current_person}"
            )
            state_utils.save_to_shared_memory(vars, current_cobot_topic=current_topic)
            logger.info(
                f"sys_mentioned_another_person: setting current_topic to {current_topic}"
            )
            state_utils.save_to_shared_memory(
                vars, current_person_occupation=occupation
            )
            logger.info(
                f"sys_mentioned_another_person: setting current_person_ocupation to {occupation}"
            )
...
simplified_dialogflow.add_system_transition(
    State.SYS_NOT_INTERESTED_IN_PERSON,
    State.USR_NOT_INTERESTED_IN_PERSON,
    usr_not_interested_in_person_response,
)

simplified_dialogflow.set_error_successor(
    State.SYS_NOT_INTERESTED_IN_PERSON, State.SYS_ERR
)
...
# workaround
simplified_dialogflow.set_error_successor(
    State.USR_NOT_INTERESTED_IN_PERSON, State.SYS_CHANGE_TO_PERSON
)
...
simplified_dialogflow.add_system_transition(
    State.SYS_AGREES_ABT_PERSON,
    State.USR_AGREES_ABT_PERSON,
    usr_agrees_abt_person_response,
)
...
# workaround
simplified_dialogflow.set_error_successor(
    State.USR_AGREES_ABT_PERSON, State.SYS_CHANGE_TO_PERSON
)
...
simplified_dialogflow.add_system_transition(
    State.SYS_DISAGREES_ABT_PERSON,
    State.USR_DISAGREES_ABT_PERSON,
    usr_disagrees_abt_person_response,
)
...
# workaround
simplified_dialogflow.set_error_successor(
    State.USR_DISAGREES_ABT_PERSON, State.SYS_CHANGE_TO_PERSON
)
...
simplified_dialogflow.add_system_transition(
    State.SYS_SAYS_OPINION_ABT_PERSON,
    State.USR_SAYS_OPINION_ABT_PERSON,
    usr_says_opinion_abt_person_response,
)
...
# workaround
simplified_dialogflow.set_error_successor(
    State.USR_SAYS_OPINION_ABT_PERSON, State.SYS_CHANGE_TO_PERSON
)
...
simplified_dialogflow.add_system_transition(
    State.SYS_MENTIONS_ANOTHER_PERSON,
    State.USR_MENTIONS_ANOTHER_PERSON,
    usr_mentions_another_person_response,
)
