...
def compose_data_for_dialogpt(recommend, ctx, actor):
    data = []
    # for uttr in dialog["utterances"][-3:]:
    #     curr_uttr = {"speaker": uttr["user"]["user_type"], "text": uttr["text"]}
    #     data.append(curr_uttr)
    ...
    if len(human_uttrs) > 0:
        if recommend == True:
            data.append(human_uttrs[-1]["text"] + ' recommendation')
        else:
            data.append(human_uttrs[-1]["text"])

    return data

def generative_response(recommend=False):
    ...
    def generative_response_handler(ctx: Context, actor: Actor, *args, **kwargs) -> Any:
        ...
        request_data = compose_data_for_dialogpt(recommend, ctx, actor)
        if len(request_data) > 0:
