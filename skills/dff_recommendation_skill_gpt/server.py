...
DIALOGPT_RESPOND_ENG_SERVICE_URL = os.environ["DIALOGPT_RESPOND_ENG_SERVICE_URL"]
...
def is_container_running():
    try:
        requested_data = {"texts": 'Boo'} #возможно здесь изменятся входные данные
        response = requests.post(DIALOGPT_RESPOND_ENG_SERVICE_URL, json={"dialog_contexts": [requested_data]}, timeout=5) #посмотреть как это же делается в инфиллинге, поменяется урл и реквест дата
