...
class State(Enum):
    ...
    SYS_RESPONSE_TO_SPEECH_FUNCTION = auto()
    USR_RESPONSE_TO_SPEECH_FUNCTION = auto()
...
patterns_supported_speech_functions = [
    "Register",
    "Check",
    "Confirm",
    "Monitor",
...
def usr_response_to_speech_function_response(vars):
    ...
    try:
        human_utterance = state_utils.get_last_human_utterance(vars)
        if word_tokenize(human_utterance["text"])[0] in aux_verbs:
            human_utterance["text"]=human_utterance["text"]+'?'
...
simplified_dialogflow.add_user_serial_transitions(
    State.USR_START,
    {
        State.SYS_RESPONSE_TO_SPEECH_FUNCTION: sys_response_to_speech_function_request,
    },
)
...
simplified_dialogflow.set_error_successor(State.USR_RESPONSE_TO_SPEECH_FUNCTION, State.SYS_ERR)
