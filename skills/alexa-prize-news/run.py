...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--host", type=str, default=os.environ.get("HOST", default=HOST), help="Host on which to run API"
    )
    parser.add_argument(
        "--port", type=int, default=os.environ.get("PORT", default=PORT), help="Port on which to run API"
    )
    parser.add_argument(
        "--dev_tg_token",
        type=str,
        default=os.environ.get("DEV_TG_TOKEN"),
        help="Telegram Token for Dev version. If None, run production version",
    )
    args = parser.parse_args()

    bot = FlaskBot(skill=Skill, bot_name="Skill", host=args.host, port=args.port)
