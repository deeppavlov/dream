...
TOPIC_PATTERNS = {
    ...
    "love": re.compile(
        r"(\blove|\bloving|\bbeloved|\bfriend|\brelationship|\brelation|\bwomen|\bmen\b" r"|\bgirlfriend|\bboyfriend)",
        re.IGNORECASE,
    ),
    "me": re.compile(r"(\babout me\b|\bmyself)", re.IGNORECASE),
    "movies": re.compile(
        r"(\bmovie|\bcinema|\bcartoon|\bseries|\btv\b|\bcinematography|\byoutube|\banime" r"|\bwatch)", re.IGNORECASE
    ),
    "news": re.compile(r"news", re.IGNORECASE),
    "politics": re.compile(r"(\bpolitic|\bpolicy)", re.IGNORECASE),
    "science": re.compile(
        r"(\bscience|\bresearch|\bsurvey|\btechnolog|\bdiscover|\bopening|\bbreakthrough" r"|\bsci( |\-)?tech)",
        re.IGNORECASE,
    ),
    "sex": re.compile(
        r"(\bsex|\bporn\b|\bporno|\bdicks?\b|\bvagina|\bpuss(y|ies)\b|\bcocks?\b"
        r"|\bmasturbat|\bpoop|\bfart(ing|ed|s)?\b|\bpenis|\bdrugs?\b|\bfucking\b)",
        re.IGNORECASE,
    ),
    ...
    "superheroes": re.compile(
        r"(\bsuperhero|\bsuper hero|\bhero(es|ism)?\b|\bmarvel|\bnemesis|\bvigilante|\bvillain"
        r"|\bsidekick|\bcomics)",
        re.IGNORECASE,
    ),
    ...
    "you": re.compile(
        r"(^(you|alexa).?$|\babout you\b|\byourself|\bsocialbot|\babout alexa\b|\byou alexa\b"
        r"|\b(this|that|amazon|current|alexa) (competition|challenge))",
        re.IGNORECASE,
    ),
}

UNUSED_TOPIC_PATTERNS = {
    ...
    "video games": re.compile(
        r"(\bvideo ?game|\bgam(e|es|ing)\b|\bplay ?station|\bplaying\b"
    ...
    "family": re.compile(
        r"(\bhusband|\bwife|\bspouse|\bfamily|\bkids?\b|\bchild\b|\bchildren"
        r"|\b(grand)?(ma|mom|mother|father|pa|dad|parent|daughters?|sons?|"
        r"child)\b)",
        re.IGNORECASE,
    ),
    "school": re.compile(
        r"(\bschool|\bstudy|\beducation|\bexams?\b|\bexamination" r"|\blearning|\bhomework)", re.IGNORECASE
    ),
