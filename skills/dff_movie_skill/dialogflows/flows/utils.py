...
MOVIE_TITLE_QUESTION = re.compile(r"(what|which)[a-zA-Z\- ']+" + MOVIE_WORDS + r"[a-zA-Z\- ']*\?", re.IGNORECASE)
...
def donot_chat_about_movies(uttr):
    if re.search(COMPILE_NOT_WANT_TO_TALK_ABOUT_IT, uttr.get("text", "").lower()) and re.search(
        MOVIE_PATTERN, uttr.get("text", "").lower()
    ):
        return True
    else:
        return False
...
def lets_chat_about_movies(uttr, prev_uttr=None):
    ...
    if (
        lets_talk_about_movies
        or chosed_topic
        or ("?" not in uttr.get("text", "") and "?" in prev_uttr.get("text", "") and curr_uttr_is_about_movies)
    ):
        return True
    else:
        return False
