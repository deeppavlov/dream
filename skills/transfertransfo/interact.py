...
def sample_sequence(personality, history, tokenizer, model, args, current_output=None):
    ...
    for i in range(args.max_length):
        ...
        probs_output.append(probs[prev.item()])

    return current_output, probs_output
...
def run():
    ...
    logger.info(f"Selected personality: {personality}")
    raw_text = "Sample a personality"
    logger.info(f"Selected personality: {tokenizer.encode(raw_text)}")
    ...
    while True:
        ...
        with torch.no_grad():
            out_ids, _ = sample_sequence(personality, history, tokenizer, model, args)
