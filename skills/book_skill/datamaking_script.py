...
def get_name(annotated_phrase, mode="author", return_plain=False):
    ...
    if mode == "author":
        class_constraints = [
            {"dataType": "aio:Entity", "value": "aio:Poet"},
        ...
        if entityname is None:
            try:
                answer = requests.request(
                    url=ENTITY_SERVICE_URL,
                    headers=headers,
                    data=json.dumps({"mention": {"text": entity}, "classConstraints": class_constraints}),
                    method="POST",
                ).json()
                ...
                if return_plain:
                    ...
                    answer = requests.request(
                        ...
                        headers=headers,
                        data=json.dumps({"query": {"text": f"query label|{entityname_plain} <aio:prefLabel> label"}}),
                        method="POST",
                    ).json()
...
def get_answer(phrase):
    headers = {"Content-Type": "application/json;charset=utf-8", "x-api-key": API_KEY}
    answer = requests.request(
        url=QA_SERVICE_URL, headers=headers, data=json.dumps({"question": phrase}), method="POST"
    ).json()
