...
class FromageModel(nn.Module):
    ...
    def forward(
        ...
        if mode == "captioning":
            ...
            if concat_captions:
                ...
                for i in range(batch_size // 2):
                    ...
                    concat_labels = torch.cat(
                        [first_labels, second_labels, first_labels_padding, second_labels_padding], axis=0
                    )  # (T*2, 768)
            ...
            output = self.lm(inputs_embeds=input_embs, labels=full_labels, output_hidden_states=True)
        elif mode == "retrieval":
            ...
            if input_prefix is not None:
                ...
                full_labels = torch.cat(
                    [torch.zeros(prefix_embs.shape[:2], dtype=torch.int64).to(labels.device) - 100, full_labels], axis=1
                )
        ...
        if mode == "retrieval":
            ...
            if not concat_captions:
                last_embedding = torch.stack(
                    [last_hidden_state[i, last_embedding_idx[i], :] for i in range(batch_size)], axis=0
                )  # (N, D)
                last_output_logit = torch.stack(
                    [output.logits[i, last_embedding_idx[i] - 1, :] for i in range(batch_size)], axis=0
                )  # (N, D)
            else:
                raise NotImplementedError
        ...
        with torch.no_grad():  # no tracking history
            ...
            for i in range(max_len):
                if "opt" in self.opt_version:
                    ...
                    if i == 0:
                        output = self.lm(
                            inputs_embeds=embeddings, use_cache=True, past_key_values=None, output_hidden_states=True
                        )
                    else:
                        output = self.lm(
                            input_ids=out[:, -1:],
    ...
    def generate_for_images_and_texts(
        ...
        if num_words == 0:
            ...
            generated_ids, generated_embeddings, _ = self.model.generate(
                input_embs, num_words, temperature=temperature, top_p=top_p, ret_scale_factor=ret_scale_factor
            )
        ...
        all_ret_idx = [i for i, x in enumerate(generated_ids[0, :] == self.model.retrieval_token_idx) if x][
            :max_num_rets
        ]
