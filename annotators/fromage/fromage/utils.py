...
def get_params_count(model, max_name_len: int = 60):
    params = [
        (name[:max_name_len], p.numel(), str(tuple(p.shape)), p.requires_grad) for name, p in model.named_parameters()
    ]
...
def get_params_count_str(model, max_name_len: int = 60):
    ...
    for name, param_count, shape, trainable in params:
        param_counts_text += (
            f'| {name:<{max_name_len}} | {"True" if trainable else "False":<10} | {shape:>15} | {param_count:>12,} |\n'
        )
    param_counts_text += "-" * (max_name_len + padding) + "\n"
    param_counts_text += (
        f'| {"Total trainable params":<{max_name_len}} | {"":<10} | {"":<15} | {total_trainable_params:>12,} |\n'
    )
    param_counts_text += (
        f'| {"Total non-trainable params":<{max_name_len}} | {"":<10} | {"":<15} | {total_nontrainable_params:>12,} |\n'
    )
    param_counts_text += "=" * (max_name_len + padding) + "\n"
