...
def validate(val_loader, model, tokenizer, criterion, epoch, args):
    ...
    def run_validate(loader, base_progress=0):
        with torch.no_grad():
            ...
            for i, (image_paths, images, caption_images, tgt_tokens, token_len) in tqdm.tqdm(
                enumerate(loader), position=0, total=len(loader)
            ):
                i = base_progress + i

                if torch.cuda.is_available():
                    tgt_tokens = tgt_tokens.load(args.gpu, non_blocking=True)
                    token_len = token_len.load(args.gpu, non_blocking=True)
                    images = images.cuda()
                ...
                for model_mode in model_modes:
                    ...
                    if model_mode == "captioning":
                        ...
                        if args.distributed:
                            ...
                            all_last_embedding = [
                                torch.zeros_like(last_embedding) for _ in range(dist.get_world_size())
                            ]
                        ...
                        all_text_features.append(last_embedding.cuda())
                        all_image_features.append(visual_embs.cuda())
                    ...
                    if model_mode == "captioning":
                        ...
                        if args.input_prompt is not None:
                            ...
                            prompt_ids = tokenizer(
                                args.input_prompt, add_special_tokens=False, return_tensors="pt"
                            ).input_ids
