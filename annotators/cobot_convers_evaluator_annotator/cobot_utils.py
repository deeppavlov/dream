...
import numpy as np
import pandas as pd
from deeppavlov.core.data import utils
...
class BasicClassificationDatasetReader(DatasetReader):
    ...
    def read(self, data_path: str, url: str = None,
        ...
        if not Path(data_path, train_file).exists():
            ...
            utils.download(source_url=url, dest_file_path=Path(data_path, train_file))
                ...
                if isinstance(x, list):
                    ...
                    if class_sep is None:
                        ...
                        try:
                            ...
                            data[data_type] = [(row[x], [float(k) for k in str(row[y]).split(class_sep)])
                                               for _, row in df.iterrows()]
                        except Exception as ex:
                            data[data_type] = [(row[x], str(row[y]).split(class_sep)) for _, row in df.iterrows()]
                            log.error("Error found: " + str(ex))
