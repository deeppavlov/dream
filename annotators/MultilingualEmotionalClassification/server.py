...
try:
    ...
    if torch.cuda.is_available():
        model.to("cuda")
        logger.info("emotion-classification is set to run on cuda")

    logger.info("emotion-classification is ready")
...
def classify_sentences(sentences):
    try:
        ...
        with torch.no_grad():
            ...
            model_output = torch.nn.functional.softmax(outputs, dim=-1).detach().cpu().numpy()
            result = []
...
def respond():
    ...
    logger.info(f"emotion-classification exec time: {total_time:.3f}s")
