...
from torch_transformers_preprocessor import (
    TorchTransformersEntityRankerPreprocessor,
)
...
class BilinearRanking(nn.Module):
    def __init__(
        self, n_classes: int = 2, emb_size: int = 768, block_size: int = 8
    ):
        super().__init__()
        ...
        self.bilinear = nn.Linear(
            self.emb_size * self.block_size, self.n_classes
        )
...
class SiameseBertElModel(nn.Module):
    ...
    def forward(
        ...
        c_cls_emb = self.encoder(
            input_ids=c_input_ids, attention_mask=c_attention_mask
        )
        softmax_scores, log_softmax = self.bilinear_ranker(
            entity_emb, c_cls_emb
        )
        ...
        if labels is not None:
            ...
            per_sample_loss = (
                -torch.bmm(
                    labels_one_hot.view(bs, 1, dim),
                    log_softmax.view(bs, dim, 1),
                )
                .squeeze(2)
                .squeeze(1)
            )
    ...
    def save(self) -> None:
        ...
        torch.save(
            {"model_state_dict": self.encoder.cpu().state_dict()},
            encoder_weights_path,
        )
        bilinear_weights_path = expand_path(
            self.bilinear_save_path
        ).with_suffix(".pth.tar")
...
class TorchTransformersEntityRankerInfer:
    def __init__(
        ...
        **kwargs,
        ...
        self.bilinear_ranking.load_state_dict(
            bilinear_checkpoint["model_state_dict"]
        )
    ...
    def __call__(
        ...
        for ii in range(num_batches):
            ...
            entity_emb_batch += (
                cur_entity_emb_batch.detach().cpu().numpy().tolist()
            )
        ...
        for (
            ...
            candidate_entities_descr_list,
        ) in zip(
            entity_emb_batch,
            candidate_entities_batch,
            ...
            if candidate_entities_list:
                ...
                descr_features = self.preprocessor(
                    candidate_entities_descr_list
                )
                ...
                entities_with_scores = [
                    ...
                    for entity, score in zip(
                        candidate_entities_list, scores_list
                    )
                ]
