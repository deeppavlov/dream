...
class EntityLinker(Component, Serializable):
    ...
    def __call__(self, entity_substr_batch: List[List[str]],
        ...
        for entity_substr_list, entity_offsets_list, entity_tags_list, sentences_list, sentences_offsets_list in \
                zip(entity_substr_batch, entity_offsets_batch, entity_tags_batch, sentences_batch, sentences_offsets_batch):
            entity_ids_list, entity_conf_list, entity_pages_list = \
                self.link_entities(entity_substr_list, entity_offsets_list, entity_tags_list, sentences_list,
                                   sentences_offsets_list)
    ...
    def link_entities(self, entity_substr_list: List[str], entity_offsets_list: List[List[int]],
                            entity_tags_list: List[str], sentences_list: List[str],
        ...
        if entity_substr_list:
            ...
            entity_substr_split_list = [[word for word in entity_substr.split(' ')
                                  ...
                                  for entity_substr in entity_substr_list]
            for entity_substr, entity_substr_split, tag in \
            ...
            for entity_ids in entity_ids_list:
                ...
                for entity_id in entity_ids:
                    ...
                    if entity_info:
                        cur_entity_id, cur_entity_label, cur_entity_descr, cur_entity_page = entity_info[0]
    ...
    def process_cand_ent(self, cand_ent_init, entities_and_ids, entity_substr_split, tag):
        if self.use_tags:
            for cand_entity_title, cand_entity_id, cand_entity_rels, cand_tag in entities_and_ids:
                if tag == cand_tag:
    ...
    def find_exact_match(self, entity_substr, tag):
        ...
        if self.lang == "@ru":
            ...
            if entity_substr_lemm != entity_substr:
                res = self.cur.execute("SELECT * FROM inverted_index WHERE title MATCH '{}';".format(entity_substr_lemm))
                entities_and_ids = res.fetchall()
                if entities_and_ids:
                    cand_ent_init = self.process_cand_ent(cand_ent_init, entities_and_ids,
                                                          entity_substr_split_lemm, tag)
        return cand_ent_init
        ...
        for word in entity_substr_split:
            ...
            if self.lang == "@ru":
                ...
                if word != word_lemm:
                    ...
                    part_entities_and_ids = res.fetchall()
                    cand_ent_init = self.process_cand_ent(cand_ent_init, part_entities_and_ids,
                                                          entity_substr_split_lemm, tag)
        return cand_ent_init
    ...
    def rank_by_connections(self, cand_ent_scores_list: List[List[Union[str, Tuple[str, str]]]]):
                    ...
                    if i != j and not already_ranked[j]:
                        ...
                        if inters:
                            ...
                            for entity2 in entities_triplets_list[j]:
                                inters = entities_triplets_list[i][entity1].intersection(entities_triplets_list[j][entity2])
        ...
        for i in range(len(entities_conn_scores_list)):
            entities_with_conn_scores_list.append(sorted(list(entities_conn_scores_list[i].items()),
