...
class EntityLinker(Component, Serializable):
    ...
    def link_entities(
        ...
        if entity_substr_list:
            ...
            for entity_substr, entity_substr_split, tag in zip(
                ...
                for entity in cand_ent_init:
                    ...
                    entities_scores = sorted(
                        entities_scores, key=lambda x: (x[0], x[1]), reverse=True
                    )
                    cand_ent_scores.append((entity, entities_scores[0]))

                cand_ent_scores = sorted(
                    cand_ent_scores, key=lambda x: (x[1][0], x[1][1]), reverse=True
                )
            ...
            if self.use_descriptions:
                substr_lens = [
                    len(entity_substr.split()) for entity_substr in entity_substr_list
                ]
    ...
    def find_exact_match(self, entity_substr, tag):
        ...
        res = self.cur.execute(
            "SELECT * FROM inverted_index WHERE title MATCH '{}';".format(entity_substr)
        )
        ...
        if self.lang == "@ru":
            entity_substr_split_lemm = [
                self.morph.parse(tok)[0].normal_form for tok in entity_substr_split
            ]
    ...
    def find_fuzzy_match(self, entity_substr_split, tag):
        ...
        for word in entity_substr_split:
            res = self.cur.execute(
                "SELECT * FROM inverted_index WHERE title MATCH '{}';".format(word)
            )
                ...
                for j in range(len(entities_for_ranking_list)):
                    if i != j and not already_ranked[j]:
                        ...
                        if inters:
                            ...
                            entities_conn_scores_list[j][entities_sets_list[j]] += len(
                                inters
                            )
                            ...
                            for entity2 in entities_triplets_list[j]:
                                inters = entities_triplets_list[i][
                                    entity1
                                ].intersection(entities_triplets_list[j][entity2])
                                inters = {elem for elem in inters if elem[0] != "P31"}
                                if inters:
                                    prev_score1 = entities_conn_scores_list[i].get(
                                        entity1, 0
                                    )
                                    prev_score2 = entities_conn_scores_list[j].get(
                                        entity2, 0
                                    )
    ...
    def rank_by_description(
        ...
        for (
            ...
            entities_with_scores = sorted(
                entities_with_scores, key=lambda x: (x[1], x[3], x[2]), reverse=True
            )
            ...
            high_conf_entities = sorted(
                high_conf_entities, key=lambda x: (x[1], x[3], x[2]), reverse=True
            )
