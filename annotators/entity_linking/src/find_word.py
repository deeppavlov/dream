...
class WordSearcher:
    def __init__(self, words_dict_filename: str, ngrams_matrix_filename: str):
    ...
    def make_ngrams_dicts(self):
        ...
        bigram_combs = list(itertools.product(letters, letters))
        bigram_combs = [''.join(comb) for comb in bigram_combs]
        trigram_combs = list(itertools.product(letters, letters, letters))
    ...
    def __call__(self, query, tags):
        ...
        query_matrix = Sparse((ngram_cnts, ngram_ids, indptr),
                              shape=(1, len(self.bigrams_dict) + len(self.trigrams_dict)))
        ...
        found_words = [word for word in found_words
                       if (word.startswith(query[0]) and abs(len(word) - len(query)) < 3
