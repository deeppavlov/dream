                ...
                for ent_first, ent_next in zip(result["response"][:-1], result["response"][1:]):
                    if f"{ent_first['text']} {ent_next['text']}" in user_nounphrases[i]:
                        if ent_first["label"] == ent_next["label"]:
                            new_result["response"] += [
                                {"text": f"{ent_first['text']} {ent_next['text']}", "label": ent_next["label"]}
                            ]
                        else:
                            new_result["response"] += [
                                {"text": f"{ent_first['text']} {ent_next['text']}", "label": "misc"}
                            ]
                        used_ent = True
                    elif not used_ent:
                        ...
                        used_ent = False
                if (
                    result["response"][-1] not in new_result["response"]
                    and result["response"][-1]["text"] not in new_result["response"][-1]["text"]
                ):
