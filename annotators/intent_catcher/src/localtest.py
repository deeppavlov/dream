...
def main_test():
    ...
    for test in tests:
        ...
        if test['intent'] is not None:
            condition = data.get(test['intent'], {'detected': 0}).get(
                'detected', 0) == 1
            condition = condition and sum(
                [v.get('detected', 0) for v in data.values()]) == 1
            if not condition:
                data = {intent: data[intent]['confidence']
                        for intent in data if data[intent]['detected']}
