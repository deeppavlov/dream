...
SKILL_NAMES_SENDING_COMMANDS = ["dff_mint_skill"]
...
def respond():
    ...
    for ann_human_uttr, ann_bot_uttr, dialog_id in zip(ann_human_utterances, ann_bot_utterances, dialog_ids):
        ...
        if active_skill in SKILL_NAMES_SENDING_COMMANDS and len(command):
            ...
            logger.info(f"mint_sender: status of sending command `{command}`: `{result}`")
        else:
            logger.info("mint_sender: NO command found in previous bot uttr")
            results += [{"human_attributes": {}}]

    total_time = time.time() - st_time
    logger.info(f"mint_sender exec time: {total_time:.3f}s")
