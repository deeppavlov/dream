    ...
    def speech_to_text_v1(self, files: List[str], config: ASRConfig) -> Dict[str, List[str]]:
        ...
        for audiofile_name in files:
            try:
                ...
                print(f"could not read this file beacuse of exception \n {e}")
                exit(-1)
        ...
        result = {"text_result": asr_results, "parameters": asdict(config)}
        ...
        if config.save_results is not None:
            ...
            if config.save_results == "json":
                ...
                with open(save_path, "w") as fp:
                    yaml.safe_dump(result, fp, default_flow_style=False, allow_unicode=True, sort_keys=False)
            else:
