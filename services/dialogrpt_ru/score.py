...
def eval_feedback(path, model, max_n=-1, max_cxt_turn=None, min_rank_gap=0.0, min_score_gap=0, max_hr_gap=1):
    """
...
def rank_hyps(path, model, max_n=-1, max_cxt_turn=None):
    ...
    for i, line in enumerate(open(path, encoding="utf-8")):
        ...
        if n % 10 == 0:
            print(
                "processed %i line, avg_hyp_score %.3f, top_hyp_score %.3f"
                % (
                    n,
                    sum_avg_score / n,
                    sum_top_score / n,
        ...
        if n == max_n:
            break
    print(
        "totally processed %i line, avg_hyp_score %.3f, top_hyp_score %.3f"
        % (
            n,
            sum_avg_score / n,
            sum_top_score / n,
...
if __name__ == "__main__":
    ...
    if args.task in ["eval_human_vs_rand", "eval_human_vs_machine"]:
        ...
        eval_feedback(
            args.data,
            model,
            max_cxt_turn=args.max_cxt_turn,
