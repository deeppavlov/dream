...
function dockercompose_cmd() {
    ...
    # else
        DOCKER_COMPOSE_CMD="docker-compose --no-ansi -p test -f docker-compose.yml -f assistant_dists/dream_reasoning/docker-compose.override.yml -f assistant_dists/dream_reasoning/test.yml"
    # fi
...
if [[ "$MODE" == "test_skills" || "$MODE" == "all" ]]; then
    ...
    for container in ranking-based-response-selector sentence-ranker \
                     dff-reasoning-skill; do
