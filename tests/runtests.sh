...
function dockercompose_cmd() {
    ...
    # else
        DOCKER_COMPOSE_CMD="docker-compose --no-ansi -p test -f docker-compose.yml -f assistant_dists/dream/docker-compose-test.override.yml -f assistant_dists/dream/test.yml"
    # fi
...
if [[ "$MODE" == "test_skills" || "$MODE" == "all" ]]; then
    ...
    for container in sentseg intent-catcher ranking-based-response-selector-fact-checking entity-linking \
                     ...
                     dff-dream-persona-chatgpt-prompted-skill dff-google-api-skill \
                     fact-checking; do
