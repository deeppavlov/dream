...
function dockercompose_cmd() {
    ...
    # else
        DOCKER_COMPOSE_CMD="docker-compose --no-ansi -p dream-alexa -f docker-compose.yml -f dev.yml -f test.yml"
    # fi
...
if [[ "$MODE" == "test_skills" || "$MODE" == "all" ]]; then
    ...
    for container in sentiment-classification movie-skill asr weather-skill program-y \
                     ...
                     knowledge-grounding dialog-breakdown knowledge-grounding-skill book-skill; do
