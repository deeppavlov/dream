...
if [[ "$MODE" == "test_dialog" || "$MODE" == "all" ]]; then
    ...
    dockercompose_cmd exec -T -u $(id -u) agent python utils/verify_compose.py -d assistant_dists/dream_russian
fi
...
if [[ "$MODE" == "test_skills" || "$MODE" == "all" ]]; then
    ...
    for container in dff-program-y-skill intent-catcher convers-evaluation-selector personal-info-skill \
                     entity-linking wiki-parser convert-reddit badlisted-words sentseg spelling-preprocessing \
                     dff-friendship-skill dff-intent-responder-skill entity-detection dialogpt dff-generative-skill; do
