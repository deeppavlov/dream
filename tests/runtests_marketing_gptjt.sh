...
function dockercompose_cmd() {
    ...
    # else
        DOCKER_COMPOSE_CMD="docker-compose --no-ansi -p test -f docker-compose.yml -f assistant_dists/marketing_assistant/docker-compose.override.yml -f assistant_dists/marketing_assistant/test.yml -f assistant_dists/marketing_assistant/proxy.yml"
    # fi
...
if [[ "$MODE" == "test_skills" || "$MODE" == "all" ]]; then
    ...
    for container in llm-based-response-selector prompt-selector dff-marketing-prompted-skill; do
