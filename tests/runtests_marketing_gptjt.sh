...
function dockercompose_cmd() {
    ...
    # else
        DOCKER_COMPOSE_CMD="docker-compose --no-ansi -p dream -f docker-compose.yml -f assistant_dists/marketing_assistant/docker-compose.override.yml -f assistant_dists/marketing_assistant/test.yml -f assistant_dists/marketing_assistant/proxy.yml"
    # fi
...
if [[ "$MODE" == "test_skills" || "$MODE" == "all" ]]; then
    ...
    for container in sentseg ranking-based-response-selector combined-classification \
                     sentence-ranker prompt-selector transformers-lm-gptjt \
                     dff-marketing-prompted-skill; do
