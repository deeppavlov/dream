...
function dockercompose_cmd() {
    ...
    # else
        DOCKER_COMPOSE_CMD="docker-compose --no-ansi -p test -f docker-compose.yml -f assistant_dists/dream_persona_ruxglm_prompted/docker-compose.override.yml -f assistant_dists/dream_persona_ruxglm_prompted/test.yml -f assistant_dists/dream_persona_ruxglm_prompted/proxy.yml"
    # fi
...
if [[ "$MODE" == "test_skills" || "$MODE" == "all" ]]; then
    ...
    for container in ranking-based-response-selector-ru dialogrpt-ru prompt-selector-ru
        transformers-lm-ruxglm dff-dream-persona-ruxglm-prompted-skill; do
