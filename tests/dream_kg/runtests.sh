...
function dockercompose_cmd() {
    ...
    # else
        DOCKER_COMPOSE_CMD="docker-compose --no-ansi -p test -f docker-compose.yml -f assistant_dists/dream_kg/docker-compose-test.override.yml -f assistant_dists/dream_kg/test.yml"
    # fi
...
if [[ "$MODE" == "test_dialog" || "$MODE" == "all" ]]; then
    ...
    dockercompose_cmd exec -T -u $(id -u) agent python3 tests/dream_kg/test_response.py
