...
def select_response(curr_candidates, curr_scores, curr_confidences, dialog):
    ...
    curr_single_cores = [(cand_scores["isResponseOnTopic"] + cand_scores["isResponseInteresting"] +
                          ...
                          cand_scores["isResponseErroneous"] + confidence)
                         for cand_scores, confidence in zip(curr_scores, curr_confidences)]

    for i, score in enumerate(curr_single_cores):
        logger.info(f'Skill {list(curr_candidates.keys())[i]} has score: {score}')
