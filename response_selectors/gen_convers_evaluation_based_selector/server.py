...
def respond():
    print("Conversation Evaluation-based Response Selector for LLM-based distributions: Beginning", flush=True)
    ...
    for i, (dialog, all_prev_active_skills) in enumerate(zip(dialogs_batch, all_prev_active_skills_batch)):
        ...
        try:
            logger.info("Curr dialog:")
            logger.info(pprint.pformat(dialog, compact=False))
            ...
            best_skill_name, best_text, best_confidence, best_human_attributes, best_bot_attributes = select_gen_response(
                curr_candidates,
                curr_scores,
                curr_confidences,
                curr_is_toxics,
...
def select_gen_response(candidates, scores, confidences, is_toxics, dialog, all_prev_active_skills=None):
